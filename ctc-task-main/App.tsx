...
function OrangeTaskApp() {
  ...
  return (
    <div className="flex h-screen bg-transparent text-gray-800 overflow-hidden font-sans selection:bg-brand-200">
      ...
      {/* Sidebar Navigation */}
      <aside className={`
        fixed inset-y-4 left-4 z-40 w-64 bg-white/70 backdrop-blur-3xl border border-white/60 shadow-2xl shadow-brand-500/10 rounded-[2rem] flex flex-col overflow-hidden transform transition-all duration-500 ease-out
      ...
      `}>
        <div className="h-full flex flex-col">
          ...
          <div className="h-20 flex items-center px-8 border-b border-white/40">
            <div className="w-10 h-10 bg-gradient-to-br from-brand-500 to-brand-400 rounded-xl flex items-center justify-center text-white mr-3 shadow-lg shadow-brand-500/30">
              <Layout size={20} className="drop-shadow-sm" />
            </div>
          ...
          <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            <div className="mb-6">
               <Button onClick={() => openCreateModal()} className="w-full justify-start gap-2 shadow-brand-200 shadow-lg" size="lg">
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col min-w-0 h-full relative lg:pl-72 transition-all duration-300">
            ...
            {currentView === 'dashboard' && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard label={t('totalTasks')} value={stats.total} icon={Layout} color="bg-purple-500" />
                  <StatCard label={t('done')} value={stats.done} icon={CheckCircle2} color="bg-success-500" />
                  <StatCard label={t('inProgress')} value={stats.inProgress} icon={Clock} color="bg-blue-500" />
                  <StatCard label={t('highPriority')} value={stats.highPriority} icon={AlertCircle} color="bg-brand-500" />
                </div>
